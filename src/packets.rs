...
impl UplinkPayload {
    pub fn from_slice(b: &[u8]) -> Result<UplinkPayload> {
        if b.len() < 9 {
            return Err(anyhow!("This is not a mesh uplink packet. : at least 9 bytes of payload are expected, got: {}. ", b.len()));
        }
...
impl EventPayload {
    pub fn from_slice(b: &[u8]) -> Result<EventPayload> {
        if b.len() < 8 {
            return Err(anyhow!("This is not a mesh event packet : at least 8 bytes of payload are expected, got: {}", b.len()));
        }
...
impl CommandPayload {
    pub fn from_slice(b: &[u8]) -> Result<CommandPayload> {
        if b.len() < 8 {
            return Err(anyhow!("This is not a mesh command packet : at least 8 bytes of payload are expected, got: {}", b.len()));
        }
        let b_len = b.len() as u64;
        let mut cur = Cursor::new(b);
        let mut ts_b: [u8; 4] = [0; 4];
...
mod test {
    ...
    fn test_mesh_packet_from_slice_too_short() {
        ...
        assert_eq!(
            ...
            "This is not a mesh uplink packet : at least 8 bytes of payload are expected, got: 1"
        );
    }
}
