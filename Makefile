...
build-mips-unknown-linux-musl:
	...
	cross +nightly build -Z build-std=panic_abort,std --target mips-unknown-linux-musl --release

build-mipsel-unknown-linux-musl:
	# mipsel is a tier-3 target.
	rustup +nightly component add rust-src
	cross +nightly build -Z build-std=panic_abort,std --target mipsel-unknown-linux-musl --release
